...
contract NFTEvolve is ERC1155, IERC1155MetadataURI, Ownable {
    ...
    mapping(uint256 => uint256) private metadataIndex;
    uint256 public id;

    event NFTEvolved(address _nftOwner, uint256 _id, uint256 _toIndex);
    event NFTPayToEvolve(address _nftOwner, uint256 _id, uint256 _toIndex, uint256 _price);
    ...
    function uri(uint256 _id) public view virtual override returns (string memory) {
        return metadatas[_id][metadataIndex[_id]];
    }
    ...
    function evolveNFT(bytes memory _params, bytes memory _messageLength, bytes memory _signature) external {
        ...
        (address _nftOwner, uint256 _id, uint256 _toIndex) = abi.decode(_params, (address, uint256, uint256));
        require(_toIndex > metadataIndex[_id], "DowngradeNotAllowed");
        metadataIndex[_id] = _toIndex;
        
        emit NFTEvolved(_nftOwner, _id, _toIndex);
    }
    ...
    function payToEvolve(bytes memory _params, bytes memory _messageLength, bytes memory _signature) external payable {
        ...
        (address _nftOwner, uint256 _id, uint256 _toIndex, uint256 _price) = abi.decode(_params, (address, uint256, uint256, uint256));
        require(_nftOwner == msg.sender, "BadSender");
        require(_price == msg.value, "BadPrice");
        require(_toIndex > metadataIndex[_id], "DowngradeNotAllowed");
        metadataIndex[_id] = _toIndex;
        
        emit NFTPayToEvolve(_nftOwner, _id, _toIndex, _price);
    }
